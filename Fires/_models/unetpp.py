...
class UnetPlusPlus(BaseLightningModule):
	...
	def __init__(self,
		...
		# Pooling layers
		self.pool_2x2 = nn.MaxPool2d(kernel_size=(2, 2))
		self.pool_3x3 = nn.MaxPool2d(kernel_size=(3, 3))

		# Upsampling layers
		self.up_2x2 = nn.Upsample(scale_factor=(2, 2), mode='bilinear', align_corners=True)
		self.up_3x3 = nn.Upsample(scale_factor=(3, 3), mode='bilinear', align_corners=True)
	...
	def forward(self, input):
		...
		x1_0 = self.conv1_0(self.pool_2x2(x0_0))
		x2_0 = self.conv2_0(self.pool_2x2(x1_0))
		x3_0 = self.conv3_0(self.pool_3x3(x2_0))
		x4_0 = self.conv4_0(self.pool_3x3(x3_0))

		# Middle layers
		x2_1 = self.conv2_1(torch.cat([x2_0, self.up_3x3(x3_0)], 1))

		x1_1 = self.conv1_1(torch.cat([x1_0, self.up_2x2(x2_0)], 1))
		x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.up_2x2(x2_1)], 1))
		
		x0_1 = self.conv0_1(torch.cat([x0_0, self.up_2x2(x1_0)], 1))
		x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, self.up_2x2(x1_1)], 1))
		x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up_2x2(x1_2)], 1))

		# Decoder layers
		x3_1 = self.conv3_1(torch.cat([x3_0, self.up_3x3(x4_0)], 1))
		x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, self.up_3x3(x3_1)], 1))
		x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, self.up_2x2(x2_2)], 1))
		x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up_2x2(x1_3)], 1))
